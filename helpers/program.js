...
const updateDBdata = async (db, mongoQuery) => {
  switch (mongoQuery.type) {
    ...
    case "update":
      return await db
        .collection(mongoQuery.collection)
        .updateMany(mongoQuery.filter, mongoQuery.update);
    case "delete":
      return await db
        .collection(mongoQuery.collection)
        .deleteMany(mongoQuery.condition);
    case "insert":
      return await db
        .collection(mongoQuery.collection)
        .insertOne(mongoQuery.insertData);
    case "create":
...
const handleDbQuery = async (queryId, db) => {
  var result = "";
  const validSemiColon = queryId.slice(-1);

  if (!isNaN(parseInt(queryId))) strQuery = queriesObj[queryId].slice(0, -1);
  else {
    if (validSemiColon === ";") strQuery = queryId.slice(0, -1);
    else result = { error: "Invalid query!" };
  }
  const mongoJson = getQueryConfig(strQuery);
  if (result === "") result = await updateDBdata(db, mongoJson);
